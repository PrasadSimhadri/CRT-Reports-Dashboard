...
export async function GET(req: NextRequest) {
  try {
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/results_sync/get_crt_testavg.aspx?course=${encodeURIComponent(
      course
    ...
    if (!apiRes.ok) {
      return new Response(
        JSON.stringify({ error: "Failed to fetch test average data" }),
        { status: 500 }
      );
    }
    ...
    return new Response(
      JSON.stringify({
        error: "Unexpected error while fetching test average data",
